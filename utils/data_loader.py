...
def build_sparse_knowledge_graph(ckg_graph, n_entities, n_items):
    rows = []
    cols = []
    data = []

    for h_id, t_id, edge_data in ckg_graph.edges(data=True):
        ...
        if h_id < n_items and t_id >= n_items:
            ...
            cols.append(h_id)
            data.append(1)
    sparse_matrix = sp.coo_matrix((data, (rows, cols)), shape=(n_entities, n_items))
...
def load_data(model_args):
    ...
    kg_sp = build_sparse_knowledge_graph(graph, n_entities, n_items)
    ...
    user_ids = train_cf[:, 0]
    item_ids = train_cf[:, 1]
    train_cf_sp = sp.csr_matrix((np.ones(len(user_ids)), (user_ids, item_ids)), shape = (n_users, n_items))
    return train_cf, train_cf_sp, test_cf, user_dict, n_params, graph, kg_sp, mean_mat_list
